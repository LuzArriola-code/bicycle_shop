...
async function cargarPapeleraGenerica(nombreTabla, titulosColumnas) {
  ...
  try {
    ...
    const respuesta = await fetch(
      `/api/${config.rutaApi}/${config.rutaListar}`,
    );
    ...
    if (respuesta.ok) {
      ...
      registros.forEach(function (registro) {
        ...
        // 2. Agregamos el botón solo si el usuario es Dueño Y la tabla está en la lista permitida
        if (
          usuarioSesion &&
          usuarioSesion.rol === "Dueño" &&
          tablasPermitidasBorrado.includes(nombreTabla)
        ) {
          accionesHTML += ` <button class="btn-borrar-definitivo" data-id="${idActual}" data-tabla="${nombreTabla}">Borrar permanente</button>`;
        }

        let tdsHTML = config.campos
          .map((campo) => `<td>${registro[campo]}</td>`)
          .join("");
    ...
    console.error(
      `Error al conectar con la papelera de ${nombreTabla}:`,
      error,
    );
  }
}
...
async function restaurarRegistro(id, nombreTabla) {
  ...
  try {
    ...
    const respuesta = await fetch(
      `/api/${config.rutaApi}/${config.rutaRestaurar}/${id}`,
      {
        method: "PUT",
      },
    );
    ...
    if (respuesta.ok) {
      ...
      const optionSeleccionada =
        selectTablas.options[selectTablas.selectedIndex];
      const columnas = optionSeleccionada
        .getAttribute("data-columnas")
        .split(",");
...
async function eliminarRegistroDefinitivo(id, nombreTabla) {
  ...
  try {
    // 2. Fetch genérico
    const respuesta = await fetch(
      `/api/${config.rutaApi}/${config.rutaBorrarDef}/${id}`,
      {
        method: "DELETE",
      },
    );
